...
# baits, onehitCDSmarker, proteome, dna output, aa output, flag for onehitCDSmarker header, help
...
my @essential = qw/baits cds proteome/;
...
# check existence of proteome file
if (!(-e $proteome)) {
die "\nERROR: Cannot find file of proteome \"$proteome\", please check --proteome ($!)\n\n";
}
...
die "\nERROR: Proteome must be in fasta format ($!)\n\n";
}
...
my $frame_info = findframes($baits_seq, $proteome, $loci_gene, $all_gene);
...
foreach my $loci_position (sort keys %$baits_seq) {
	...
	my $ori_loci_position = $loci_position;
	...
	if (exists $loci_gene->{$loci_position}) {
		...
		print STDERR "\nWARNING: Protein of reference gene ($gene) of loci $ori_name was absent in $proteome\n";
		}
	} else {
	print STDERR "\nWARNING: No reference gene found for loci $ori_name\n";
	}
	
	push @uncorrected_loci, $ori_loci_position if (! $print);
}

foreach my $loci_position (@uncorrected_loci) {
	my $uncorrected_seq = $baits_seq->{$loci_position}->{seq};
	my $ori_name = $baits_seq->{$loci_position}->{header};
	...
	print STDERR "\nWARNING: No appropriate frame found for loci $ori_name\n";	
	}
}
...
This is a script to predict frame and generate coding and amino acid sequences of baits sequences.

Overview:
Each baits sequence will be first aligned with its corresponding reference protein to determine its frame. If baits and reference protein is too diverged or reference protein is not found, frame with least stop codon will be kept
...
Example usage:
	...
	perl predict_frames.pl --baits species.fas --cds species.onehitCDSmarkers.column1.txt --proteome species.pep.fas
...
--cds
  OnehitCDSmarker generated from Evolmarker. Only info in first columns will be used, so just input file with only first column is fine
--proteome
